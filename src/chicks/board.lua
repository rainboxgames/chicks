...
function Board:initialize(map)
    assert(#map == self.MAX_TILES, "Map must contain exactly " .. Board.MAX_TILES .. " tiles")
    for k, v in pairs(map) do
        assert(v == self.MARBLES.empty
            or v == self.MARBLES.blue
            or v == self.MARBLES.green
            or v == self.MARBLES.yellow
            or v == self.MARBLES.purple
            or v == self.MARBLES.white
            or v == self.MARBLES.red,
            "Invalid marble")
    end
    self.__map = map
end
...
function Board:place(position, marble)
    assert(1 <= position and position <= MAX_TILES,
        "Invalid position")
    assert(marble == self.MARBLES.empty
        or marble == self.MARBLES.blue
...
function Board.static:axis_rotate_cw(xyz)
    return {
        x = xyz.z,
        y = xyz.x,
        z = self.ROTATION_OFFSET - xyz.y
    }
end

function Board.static:axis_rotate_ccw(xyz)
    return {
        x = xyz.y,
        y = self.ROTATION_OFFSET - xyz.z,
        z = xyz.x
    }
end

function Board.static:axis_intersect(xyz)
    local s = {}
    if xyz.x ~= nil and xyz.y ~= nil then
        local sx = {}
        for _, v in pairs(self.X_AXIS[xyz.x]) do sx[v] = true end
        for _, v in pairs(self.Y_AXIS[xyz.y]) do
            if sx[v] then s[#s+1] = v end
        end
    elseif xyz.y ~= nil and xyz.z ~= nil then
        local sy = {}
        for _, v in pairs(self.Y_AXIS[xyz.y]) do sy[v] = true end
        for _, v in pairs(self.Z_AXIS[xyz.z]) do
            if sy[v] then s[#s+1] = v end
        end
    elseif xyz.z ~= nil and xyz.x ~= nil then
        local sz = {}
        for _, v in pairs(self.Z_AXIS[xyz.z]) do sz[v] = true end
        for _, v in pairs(self.X_AXIS[xyz.x]) do
